...
def desc(request, myid):
    product = models.Product.objects.all().filter(id=myid)
    if not product:
        return render(request, '404.html')
    productreview = models.Product.objects.get(id=myid)
    review = models.Review.objects.filter(product=productreview)
    print(review)
    # for i in review:
    #     print(i.comment)
    # context = {
    #     'reviews': review,
    # }
    return render(request, 'desc.html', {'product': product[0], "reviews": review})
...
def review(request):
    ...
    print("printing")
    # print(prod_id, comment, rating)
    user = request.user
    models.Review(user=user, product=product,
                  comment=comment, rate=rating).save()
    print("donw")

    product = models.Product.objects.all().filter(id=prod_id)

    # print(product.objects.values_list())
